...
Return the result in this structure. Keep it markdown so it can feed the next skill directly. Never use em dashes. All three personas run every time: open with the confirmation line, then show Chase's strategy, then the two Parts.

## Required first line of every run
...
- Loop grades + Marg's key critiques + Expert's changes.
- Strongest opener / closer / transplant card; cut or fix candidates.
- Which dimensions cap below A+ and why.
...
## Appendix
- Where the honesty guardrail blocked a juiced grade.
- Real proof that belongs on web or email instead of the store.
- Optional personalization notes.

## Rules for the output
- Markdown only by default. Offer a .docx (via the docx skill) only if asked.
