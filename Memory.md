...
5. **No em dashes anywhere in any company output, full stop. This is absolute. Check every single response, every time, with no exceptions for drafts or casual chat text.**
   ...
   - **The 2026-09-05 repo-wide sweep missed `.claude/skills/`. Cleared 2026-09-09.**
     The ten WeStretch-owned skill folders there (`daily-brief*`,
     `app-store-specialist-*`, `asana-backlog-triage`, `cmo-*`) still held about
     190 em dashes, including in the `description:` frontmatter that shows up in
     the skill picker. All swept during the 2026-09-09 daily brief run, verified
     word-for-word identical apart from punctuation and sentence capitalization.
     The other 57 folders under `.claude/skills/` are NTFS junctions into
     `.agents/skills/` (vendored packages, roughly 5,000 em dashes) and stay out
     of scope per the rule below. When checking coverage, remember `os.path.islink`
     does not detect NTFS junctions; use `dir /AL` instead.
